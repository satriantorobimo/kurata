...
import { resendBrokerPasswordSetupEmail, updateAssetReviewStatus, updateFormReviewStatus, type ReviewActionResult } from "@/app/admin/actions";
...
export function AdminConsole({ initialData }: { initialData: AdminData }) {
  ...
  useEffect(() => {
    if (setupResendCooldown <= 0) return;
    const interval = window.setInterval(() => setSetupResendCooldown((seconds) => Math.max(0, seconds - 1)), 1000);
  ...
  const resendBrokerSetup = () => {
    if (collection !== "brokers" || !selected) return;
    startTransition(async () => {
      const result = await resendBrokerPasswordSetupEmail(selected.entityId ?? selected.id);
      setNotice(result.message);
      if (result.retryAfterSeconds) setSetupResendCooldown(result.retryAfterSeconds);
    });
  };
