...
const SECTION_META: Record<Collection, { title: string; eyebrow: string; icon: typeof UsersRound }> = {
  ...
  brokers: { title: "Verifikasi Broker", eyebrow: "Kemitraan broker", icon: UserRoundCheck },
  assets: { title: "Persetujuan Aset", eyebrow: "Listing broker", icon: ClipboardCheck },
...
export function AdminConsole() {
  ...
  const nav = [
    ...
    { id: "brokers" as const, label: "Broker", icon: UserRoundCheck },
  ...
  return <div className="min-h-screen bg-background pt-20">
    <div className="container-main py-8 md:py-12">
      <div className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-label-sm font-label-sm uppercase tracking-wider text-primary">Kurata workspace</p><h1 className="mt-2 text-3xl font-bold tracking-tight text-on-surface md:text-4xl">Pusat Administrasi</h1><p className="mt-2 max-w-2xl text-body-md leading-6 text-on-surface-variant">Tinjau pengguna, kemitraan broker, listing, dan konten dari satu tempat.</p></div><span className="inline-flex w-fit items-center gap-2 rounded-full border border-amber-300 bg-amber-50 px-3 py-2 text-label-sm text-amber-900"><ShieldCheck className="h-4 w-4" />Mode demo — belum terhubung ke akun produksi</span></div>
      ...
      <div className="grid gap-6 lg:grid-cols-[230px_minmax(0,1fr)]"><aside className="h-fit rounded-2xl border border-border-subtle bg-surface-container-lowest p-3 shadow-card"><nav className="flex gap-1 overflow-x-auto lg:flex-col">{nav.map(({ id, label, icon: Icon }) => <button key={id} onClick={() => { setSection(id); setSelectedId(null); setQuery(""); }} className={cn("flex shrink-0 items-center gap-3 rounded-xl px-4 py-3 text-left text-label-md transition-colors", section === id ? "bg-primary text-on-primary" : "text-on-surface-variant hover:bg-surface-container-low hover:text-primary")}><Icon className="h-5 w-5" />{label}{id !== "overview" && <span className={cn("ml-auto rounded-full px-2 py-0.5 text-[11px]", section === id ? "bg-white/20" : "bg-surface-container-high")}>{data[id].filter((item) => item.status === "pending" || item.status === "under_review").length}</span>}</button>)}</nav></aside>
        <main>{section === "overview" ? <Overview data={data} pendingCount={pendingCount} onNavigate={(next) => setSection(next)} /> : <ReviewSection title={SECTION_META[section].title} eyebrow={SECTION_META[section].eyebrow} records={filtered} total={records.length} query={query} onQuery={setQuery} selected={selected} onSelect={setSelectedId} onUpdate={updateStatus} approveStatus={section === "assets" || section === "content" ? "published" : "verified"} />}</main>
      </div>
    </div>
  </div>;
}
...
function Overview({ data, pendingCount, onNavigate }: { data: typeof ADMIN_DEMO_DATA; pendingCount: number; onNavigate: (section: Collection) => void }) {
  const cards: { section: Collection; label: string; value: number; hint: string; icon: typeof UsersRound }[] = [
    ...
    { section: "brokers", label: "Aplikasi broker", value: data.brokers.filter((x) => x.status === "pending" || x.status === "under_review").length, hint: "Menunggu keputusan kemitraan", icon: UserRoundCheck },
  ...
  return <><section className="rounded-2xl bg-primary p-6 text-on-primary md:p-8"><p className="text-label-sm font-label-sm uppercase tracking-wider text-on-primary-container">Prioritas hari ini</p><div className="mt-3 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><h2 className="text-3xl font-bold">{pendingCount} item perlu ditinjau</h2><p className="mt-2 max-w-xl leading-6 text-on-primary/80">Selesaikan pemeriksaan pengguna, broker, dan aset agar proses di Kurata tetap transparan dan terjaga.</p></div><button onClick={() => onNavigate("users")} className="rounded-xl bg-white px-4 py-3 text-label-md font-label-md text-primary hover:bg-surface-container-low">Mulai tinjau</button></div></section><section className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{cards.map(({ section, label, value, hint, icon: Icon }) => <button key={section} onClick={() => onNavigate(section)} className="rounded-2xl border border-border-subtle bg-surface-container-lowest p-5 text-left shadow-card transition-transform hover:-translate-y-0.5"><Icon className="h-6 w-6 text-primary" /><p className="mt-5 text-3xl font-bold text-on-surface">{value}</p><h3 className="mt-1 text-label-md font-label-md text-on-surface">{label}</h3><p className="mt-1 text-label-sm leading-5 text-on-surface-variant">{hint}</p></button>)}</section><section className="mt-8 rounded-2xl border border-border-subtle bg-surface-container-lowest p-6"><div className="flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-primary" /><div><h2 className="font-bold text-on-surface">Prinsip review Kurata</h2><p className="mt-1 text-body-md text-on-surface-variant">Setiap keputusan perlu alasan yang jelas, catatan internal, dan riwayat audit saat database produksi dihubungkan.</p></div></div></section></>;
}
