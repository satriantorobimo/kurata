...
export function AssetReviewWorkspace({ asset }: { asset: ReviewRecord }) {
  ...
  return <div className="min-h-screen bg-background pt-20">
    <div className="container-main py-8 md:py-12">
      ...
      {decision && <div className={cn("mt-6 flex items-start gap-3 rounded-xl border p-4 text-body-md", decision === "approved" ? "border-primary/20 bg-primary/5 text-primary" : "border-amber-200 bg-amber-50 text-amber-900")} role="status">{decision === "approved" ? <CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0" /> : <CircleAlert className="mt-0.5 h-5 w-5 shrink-0" />}<span>{decision === "approved" ? "Aset ditandai siap tayang. Perbarui status di Pusat Administrasi untuk menerbitkannya." : decision === "changes" ? "Permintaan perbaikan dicatat. Beri tahu Mitra Kurata melalui kanal komunikasi." : "Penolakan dicatat."}</span></div>}
