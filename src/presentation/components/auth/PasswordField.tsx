...
export function PasswordField({ id, name, label, error, autoComplete, hint }: { id: string; name: string; label: string; error?: string; autoComplete: "current-password" | "new-password"; hint?: string }) {
  ...
  return (
    <div>
      ...
      <div className="relative mt-2">
        <input id={id} name={name} type={visible ? "text" : "password"} required minLength={8} autoComplete={autoComplete} aria-invalid={Boolean(error)} aria-describedby={error ? `${id}-error` : undefined} className="w-full rounded-lg border border-border-subtle bg-surface-container-lowest px-3 py-3 pr-11 text-body-md text-on-surface outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20 aria-[invalid=true]:border-error" />
