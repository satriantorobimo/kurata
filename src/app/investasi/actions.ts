...
export async function submitInvestmentInquiry(_previousState: InvestmentInquiryFormState, formData: FormData): Promise<InvestmentInquiryFormState> {
  ...
  if (area.length < 2 || area.length > 160) fieldErrors.area = "Masukkan area potensi lahan yang diminati.";
  if (budget.length < 2 || budget.length > 100) fieldErrors.budget = "Masukkan kisaran anggaran potensi lahan.";
  if (!isAllowed(objective, INVESTMENT_OBJECTIVES)) fieldErrors.objective = "Pilih tujuan Anda.";
  if (!isAllowed(horizon, INVESTMENT_HORIZONS)) fieldErrors.horizon = "Pilih jangka waktu Anda.";
  ...
  return { status: "success", message: "Permintaan konsultasi potensi lahan Anda sudah diterima untuk ditinjau.", reference: result.id };
}
