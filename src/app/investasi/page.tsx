...
export default async function InvestmentPage() {
  ...
  return <div className="min-h-screen bg-background pt-16 md:pt-20">
    <section className="relative overflow-hidden bg-surface-container-low py-14 md:py-20">
      <div className="container-main relative">
        <div className="max-w-2xl">
          <div className="inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-2 text-label-sm font-label-sm text-primary"><MapPin className="h-4 w-4" />Potensi Lahan</div>
          <h1 className="mt-5 text-3xl font-bold leading-tight tracking-tight text-on-surface sm:text-4xl md:text-5xl">Temukan Lahan dengan Potensi Bisnis Terbaik</h1>
          <p className="mt-5 text-body-md leading-7 text-on-surface-variant">Jelajahi lahan pilihan yang telah dipisahkan khusus berdasarkan potensi bisnis, akses, dan peluang pengembangannya.</p>
    ...
    <main className="container-main py-10 md:py-14">
      ...
      <section aria-labelledby="listing-title" className="mt-14">
        <p className="text-label-sm font-label-sm uppercase tracking-wider text-primary">Rekomendasi untuk Anda</p>
        <h2 id="listing-title" className="mt-2 text-2xl font-bold text-on-surface">Lahan Potensial untuk SPBU</h2>
        {listingResult.properties.length === 0 ? <p className="mt-7 rounded-xl border border-dashed border-outline-variant bg-surface-container-low p-8 text-body-md text-on-surface-variant">Belum ada listing Potensi Lahan yang diterbitkan.</p> : <div className="mt-7 grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
